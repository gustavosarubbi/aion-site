...
import { ChartLineUp, MagnifyingGlass, Palette, ShieldCheck, TrendUp } from "@phosphor-icons/react";

const montserrat = { fontFamily: "var(--font-montserrat)" } as const;

const pillarIcons = {
  msoeawqm: MagnifyingGlass,
  wloilxuq: Palette,
  hwuyodym: ChartLineUp,
  shield2: ShieldCheck,
} as const;
...
type Pillar = {
  ...
  desc: string;
  tag: string;
  iconId: keyof typeof pillarIcons;
  image: string;
  accentColor: string;
  delay: number;
};
...
{/* ── Abstract Tech Geometric Background ── */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        {/* Layered technical gradient texture */}
        <div
          className="absolute inset-0 opacity-[0.36]"
          style={{
            backgroundImage:
              "radial-gradient(circle at 20% 20%, rgba(55,156,253,0.24), transparent 42%), radial-gradient(circle at 85% 68%, rgba(93,184,255,0.22), transparent 40%), linear-gradient(140deg, rgba(4,10,20,0.95) 0%, rgba(9,20,38,0.88) 50%, rgba(5,10,20,0.95) 100%)",
          }}
        />
...
type CardPilarProps = Pillar;

function CardPilar({ title, desc, tag, iconId, image, accentColor, delay }: CardPilarProps) {
  const Icon = pillarIcons[iconId] ?? ChartLineUp;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-30 flex flex-col h-full p-6 sm:p-7">
        {/* Icon */}
        <div className="w-14 h-14 rounded-full bg-[#0a1628] border border-[#379cfd]/20 flex items-center justify-center mb-6 self-start group-hover:scale-110 group-hover:border-[#379cfd]/60 group-hover:bg-[#0d1f3c] transition-all duration-500 shadow-[0_0_20px_rgba(55,156,253,0.08)] overflow-hidden">
          <motion.div
            className="flex items-center justify-center w-full h-full"
            whileHover={{ scale: 1.1 }}
            transition={{ type: "spring", stiffness: 400, damping: 20 }}
          >
            <Icon size={30} weight="duotone" className="text-[#5db8ff] drop-shadow-[0_0_14px_rgba(55,156,253,0.5)]" />
          </motion.div>
        </div>
