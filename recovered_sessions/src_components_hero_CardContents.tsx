...
export function CodeCardContent({
    ...
    const lineCount = reducedDetail ? 5 : 7;
        ...
        <group position={[-1.2, 0.5, 0.15]}>
            ...
            {[...Array(lineCount)].map((_, i) => (
                <mesh key={`ln${i}`} position={[-0.15, -i * 0.22, 0]}>
                    ...
                    <meshStandardMaterial color="#94a3b8" emissive="#94a3b8" emissiveIntensity={0.35} />
                </mesh>
            ))}
            ...
            {[
                ...
                <InteractiveDetail key={i} offsetMs={i * 34} reducedDetail={reducedDetail} enabled={animateDetails}>
                    {(pulse) => (
                        <mesh position={[line.w / 2 + line.indent + 0.1, -i * 0.22, 0]}>
                            <boxGeometry args={[line.w, 0.06 + pulse * 0.08, 0.04 + pulse * 0.05]} />
                            <meshStandardMaterial color={line.color} emissive={line.color} emissiveIntensity={0.6 + pulse * 2} />
                        </mesh>
                    )}
                </InteractiveDetail>
            ))}
            ...
            <mesh position={[0.55, -1 * 0.22, 0.01]}>
                ...
                <meshStandardMaterial color="#e2e8f0" emissive="#e2e8f0" emissiveIntensity={0.7} />
            </mesh>
        </group>
    );
}
...
export function PreviewCardContent({
    ...
    return (
        <group position={[0, -0.1, 0.15]}>
            ...
            <InteractiveDetail offsetMs={18} reducedDetail={reducedDetail} enabled={animateDetails}>
                {(pulse) => (
                    <group position={[-0.8, 0.45, 0.03]}>
                        <mesh>
                            <boxGeometry args={[1.0 + pulse * 0.1, 0.12, 0.01]} />
                            <meshStandardMaterial color="#38bdf8" emissive="#38bdf8" emissiveIntensity={0.6 + pulse * 1.5} />
                        </mesh>
            ...
            {[0.2, 1.0].map((x, i) => (
                <InteractiveDetail key={i} offsetMs={i * 40 + 60} reducedDetail={reducedDetail} enabled={animateDetails}>
                    {(pulse) => (
                        <group position={[x, -0.3, 0.02]}>
                            ...
                            <mesh position={[0, 0.25, 0.025 + pulse * 0.04]}>
                                <circleGeometry args={[0.12, reducedDetail ? 16 : 32]} />
                                <meshStandardMaterial
                                    color={i === 0 ? "#818cf8" : "#34d399"}
                                    emissive={i === 0 ? "#818cf8" : "#34d399"}
                                    emissiveIntensity={0.55 + pulse * 2}
                                />
                            </mesh>
    ...
    return (
        <group position={[0, -0.4, 0.15]}>
            ...
            {[
                ...
                <InteractiveDetail key={i} offsetMs={i * 36 + 24} reducedDetail={reducedDetail} enabled={animateDetails}>
                    {(pulse) => (
                        <group position={[bar.x, -0.2 + (bar.h + pulse * 0.25) / 2, 0.02]}>
                            ...
                            <mesh position={[0, (bar.h + pulse * 0.25) / 2, 0.01]}>
                                ...
                                <meshStandardMaterial color={baseColor} emissive={baseColor} emissiveIntensity={0.6 + pulse * 2.5} />
                            </mesh>
                        </group>
                    )}
                </InteractiveDetail>
            ...
            <InteractiveDetail offsetMs={200} reducedDetail={reducedDetail} enabled={animateDetails}>
                {(pulse) => (
                    <mesh position={[-0.1, 0.55, 0.04 + pulse * 0.06]} rotation={[0, 0, 0.25]}>
                        ...
                        <meshStandardMaterial
                            color="#22d3ee"
                            emissive="#22d3ee"
                            emissiveIntensity={0.7 + pulse * 3.5}
                            transparent
